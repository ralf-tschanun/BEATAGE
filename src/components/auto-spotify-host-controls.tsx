...
export function AutoSpotifyHostControls({
  ...
  useEffect(() => {
    if (pausedByHost || disabled || connected !== true) {
      clearDebounce();
      return;
    }
    ...
    <section className="space-y-3 rounded-2xl border border-border/60 p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={!pausedByHost}
            disabled={disabled || busy || connected !== true}
            onChange={(event) => {
              if (event.target.checked) onContinue();
              else {
                setPausedByHost(true);
                clearDebounce();
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          type="button"
          variant="outline"
          disabled={disabled || busy || !pausedByHost}
          onClick={onContinue}
        >
          Continue
        </Button>
