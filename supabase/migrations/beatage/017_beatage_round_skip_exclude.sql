-- Host can skip an active round (no scoring) or exclude a past revealed round from scoring.

alter table public.beatage_rounds
  drop constraint if exists beatage_rounds_status_check;

alter table public.beatage_rounds
  add constraint beatage_rounds_status_check
  check (status in ('pending', 'active', 'revealed', 'skipped', 'excluded'));

-- Dashboard rank: only count revealed rounds (excluded + skipped stay out).
