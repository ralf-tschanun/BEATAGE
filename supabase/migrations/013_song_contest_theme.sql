...
drop function if exists public.nominate_candidate(uuid, text, text, text);
...
begin
  ...
  if coalesce(v_contest.theme, 'generic') = 'song' then
    ...
    -- Song contests only store title + artist (+ optional catalog link).
    v_description := null;
  end if;
