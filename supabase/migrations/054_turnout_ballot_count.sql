-- Track how many question ballots each voter has submitted (Anything multi-question).

alter table public.ballot_turnout
  add column if not exists ballot_count integer not null default 1
    check (ballot_count > 0);

-- Backfill from existing ballots when possible.
update public.ballot_turnout t
set ballot_count = greatest(
  1,
  (
    select count(*)::integer
    from public.ballots b
    where b.contest_id = t.contest_id
      and b.voter_user_id = t.voter_user_id
  )
);
